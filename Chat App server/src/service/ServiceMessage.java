...
public class ServiceMessage {
    ...
    // SQL statements
    private static final String INSERT_TEXT_MESSAGE
            = "INSERT INTO message (MessageType, FromUserID, ToUserID, Text, Time, FileID, BoxID) VALUES (?, ?, ?, ?, ?, NULL, ?)";

    private static final String INSERT_FILE_MESSAGE
            = "INSERT INTO message (MessageType, FromUserID, ToUserID, Text, Time, FileID, BoxID) VALUES (?, ?, ?, NULL, ?, ?, ?)";
    ...
    private static final String SELECT_MESSAGES_BY_BOX
            = "SELECT * FROM message WHERE BoxID = ? ORDER BY Time ASC";
    ...
    public void saveTextMessage(Model_Send_Message message) throws SQLException {
        try (PreparedStatement p = con.prepareStatement(INSERT_TEXT_MESSAGE)) {
            ...
            p.setInt(6, message.getBoxid());  // Set BoxID
    ...
    public List<Model_Send_Message> getMessagesByBox(int boxID) throws SQLException {
        List<Model_Send_Message> messages = new ArrayList<>();

        try (PreparedStatement p = con.prepareStatement(SELECT_MESSAGES_BY_BOX)) {
            p.setInt(1, boxID);  // Set BoxID
            ...
            try (ResultSet rs = p.executeQuery()) {
                while (rs.next()) {
                    ...
                    message.setBoxid(rs.getInt("BoxID"));  // Set BoxID
                    messages.add(message);
                }
            }
        }
